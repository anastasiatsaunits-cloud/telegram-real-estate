  ...
  return (
    <main style={{ minHeight: '100vh', padding: '24px 20px 40px', fontFamily: 'Inter, Arial, sans-serif', color: '#1f1f1f' }}>
      ...
      <section style={{ background: '#ffffff', borderRadius: 24, padding: 24, boxShadow: '0 10px 30px rgba(0,0,0,0.05)' }}>
        ...
        <p style={{ margin: '0 0 20px', color: '#5c5348', lineHeight: 1.5 }}>Живой экран выбора региона для miniapp MVP.</p>

        <div style={{ display: 'grid', gap: 12 }}>
          {regions.map((region) => (
            <button key={region.id} type="button" style={{ textAlign: 'left', border: '1px solid #eadfce', borderRadius: 18, padding: 18, background: '#fffaf6', cursor: 'pointer', fontSize: 17, fontWeight: 600 }}>
              {region.name}
            </button>
          ))}
        </div>
